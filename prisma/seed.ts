...
const main = async () => {
      ...
      data: {
        ...
        createdCourses: {
          create: {
            name: "React.js",
            createdAt: faker.date.past(),
            presentation: "Formation à React.js V18",
            image:
    ...
    users.push(
      await prisma.user.create({
        data: {
          ...
          email: faker.internet.email(),
          createdAt: faker.date.past(),
          image: `https://api.dicebear.com/7.x/adventurer/svg?seed=${faker.lorem.words(
            3
          )}`,
          createdCourses: {
            create: {
  ...
  for (const course of courses) {
    const random3Users = faker.helpers.arrayElements(users, getRandomInt(20));
